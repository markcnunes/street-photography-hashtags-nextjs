...
export default function Contribution() {
  const { data: categories, error } = useSWR('/api/categoriesData', fetcher);

  if (error) return <LayoutDefault>Failed to load</LayoutDefault>;
  if (!categories) return <LayoutDefault>Loading...</LayoutDefault>;
    ...
    <LayoutDefault title='Suggesting new keywords'>
      {
        <StyledDescription>
          <p>
            Contributions are welcomed! If you have suggestions to improve some
            of the categories, please let's know.
          </p>
          <h3>Contribution Guidelines</h3>
          <p>
            Please read our{' '}
          ...
          <p>
            Only hashtags are accepted. Mentions for keywords won't be allowed
            because these accounts most of time have hastags as well. It will
            help us to filter duplications and keep the list simple. Ensure the
            keyword has good visibility. Wait at least 30 days if it is a
            newborn keyword before submitting it, to give it a chance to mature.
            Also, check if it doesn't exist in some of the categories, as yours
            may be a duplicate, you can send the suggestions to{' '}
            <a
              href='https://www.instagram.com/markcnunes/'
              title="Mark's website"
              target='_blank'
          ...
          <p>Thanks for helping! 😎</p>
          <h3>Verify if keyword exist</h3>
          <VerifyKeyword keywords={categories} />
        </StyledDescription>
      }
    </LayoutDefault>
  );
...
//   const res = await fetch(`${NEXT_HTTP + VERCEL_URL}/api/categoriesData`);
